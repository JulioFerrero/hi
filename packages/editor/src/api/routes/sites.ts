...
export const sitesRoute = new Hono()
  ...
  .post(
    ...
    async (c) => {
      ...
      const [row] = await db.insert(sites).values({
        id: nanoid(),
        slug: body.slug,
        data: (body.data ?? { name: body.slug }) as any, // eslint-disable-line @typescript-eslint/no-explicit-any
      }).returning();
  ...
  .patch(
    ...
    async (c) => {
      const body = c.req.valid("json");
      const row = await updateById(sites, c.req.param("id"), { slug: body.slug, data: body.data as any }); // eslint-disable-line @typescript-eslint/no-explicit-any
      if (!row) return c.json({ error: "Not found" }, 404);
      return c.json(row);
    }
  )

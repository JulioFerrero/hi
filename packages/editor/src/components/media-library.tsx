...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@hi/ui/dialog";
...
export function MediaLibrary({ open, onClose, onSelect, siteId, api }: MediaLibraryProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) { setSelectedUrl(null); onClose(); } }}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-white/80 text-base">Media Library</DialogTitle>
        </DialogHeader>

        <div className="flex-1 overflow-y-auto editor-scroll space-y-4">
          <div
            className={cn(
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 h-6 w-6 flex items-center justify-center rounded-lg bg-black/60 text-white/50 hover:text-red-400 hover:bg-black/80 opacity-0 group-hover:opacity-100 transition-all"
                    >
